...
from bokeh.models import ColumnDataSource, Legend, Span, Label
...
class UBXScope:
  def __init__(self, inputBuffer):
    ...
    for block in range(self.numRfBlocks):
      dataSourceDict[f'spectrumBinCenterFreqs_{block}'] = []
      dataSourceDict[f'spectrumMaxima_{block}'] = []
      dataSourceDict[f'spectrum_{block}'] = []
    ...
    for block in range(self.numRfBlocks):
      ...
      #Metadata label
      self.blockMetadataLabels[block] = Label(text=f'PGA Gain: X dB',x=10, y=10, x_units='screen', y_units='screen', text_font_size='10px')
      self.spectrumFigures[block].add_layout(self.blockMetadataLabels[block])

    #Simple row layout
    self.doc.add_root(row(children=self.spectrumFigures, sizing_mode="stretch_both"))
    ...
    if msgClass == 'SPAN':
      ...
      for block in range(msg.numRfBlocks):
        ...
        try:
          ...
          newSpectrumData[f'spectrumMaxima_{block}'] = newSpectrumData[f'spectrum_{block}']
        ...
        try:
          ...
          newSpectrumData[f'spectrumCMA_{block}'] = newSpectrumData[f'spectrum_{block}']
